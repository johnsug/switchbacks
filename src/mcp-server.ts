...
/**
 * Build and register all MCP tools for a given user config + cache.
 * Used by both stdio (local) and HTTP/SSE (hosted) entry points.
 */
export function buildMcpServer(config: Config, cache: Cache): McpServer {
  ...
  server.tool(
    ...
    async (params) => {
      const result = await getRecentRuns(params, cache, config);
  ...
  server.tool(
    ...
    async (params) => {
      const result = await getFitnessTrend(params, cache, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const result = await getActivityDetail(params, cache, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const result = await comparePeriods(params, cache, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const result = await getWeatherImpact(params, cache, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const result = await estimateRaceTime(params, cache, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const result = await getIntervalAnalysis(params, config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
